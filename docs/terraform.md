...
| <a name="input_max_days_count"></a> [max\_days\_count](#input\_max\_days\_count) | How many days Docker Images AWS ECR will store. Only used when [time\_based\_rotation](#time\_based\_rotation) is true | `number` | `180` | no |
| <a name="input_max_image_count"></a> [max\_image\_count](#input\_max\_image\_count) | How many Docker Image versions AWS ECR will store. Only used when [count\_based\_rotation](#count\_based\_rotation) is true | `number` | `500` | no |
...
| <a name="input_count_based_rotation"></a> [count\_based\_rotation](#count\_based\_rotation) | Set to true to filter image based on the `imageCountMoreThan` count type. | `bool` | `true` | no |
| <a name="input_time_based_rotation"></a> [time\_based\_rotation](#time\_based\_rotation) | Set to true to filter image based on the `sinceImagePushed` count type. | `bool` | `false` | no |
