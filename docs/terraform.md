...
| <a name="input_time_based_rotation"></a> [time\_based\_rotation](#time\_based\_rotation) | Set 'true' to filter image based on the `sinceImagePushed` count type. [max\_image\_count](#input\_max\_image\_count) coresponds to number of days before expiration | `bool` | `false` | no |
